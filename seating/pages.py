    ...
    def vars_for_template(self):
        ...
        for p in self.subsession.get_players():
            if p.claim_it:
                ...
                count_claimed+= 1
        if count_claimed < self.session.config["section_seats"]:
            self.player.claim_it = True
            return {"name":SUNet_to_name[self.participant.label],"got_it":True}
        else:
            self.player.claim_it = False
            return {"name":SUNet_to_name[self.participant.label],"got_it":False}
