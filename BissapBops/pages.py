...
class Introduction(Page):
    ...
    def vars_for_template(self):
        ...
        return {"reading_limit":Constants.reading_time,"total_time":total_time,"planning_doc_time":Constants.planning_doc_time_minutes,"negotiating_time":Constants.negotiating_time}




class USAMart_materials(Page):
    form_model = "player"
    ...
    def vars_for_template(self):
        return {"pdf_file":"BissapBops/Bissap Bops - USA Mart.pdf","xlsx_file":"BissapBops/USA-Mart points.xlsx"}


class NJLLC_materials(Page):
    form_model = "player"
    ...
    def vars_for_template(self):
        return {"pdf_file":"BissapBops/Bissap Bops - NJ LLC.pdf","xlsx_file":"BissapBops/NJ points.xlsx"}
...
class Planning_doc(Page):
    ...
    def vars_for_template(self):
        if self.player.id_in_group == 1:
            return {"pdf_file":"BissapBops/Bissap Bops - USA Mart.pdf", "xlsx_file":"BissapBops/USA-Mart points.xlsx", "max_word_limit":Constants.planning_doc_length}
        if self.player.id_in_group == 2:
            return {"pdf_file":"BissapBops/Bissap Bops - NJ LLC.pdf", "xlsx_file":"BissapBops/NJ points.xlsx", "max_word_limit":Constants.planning_doc_length}
...
class Meeting_location_reminder(Page):
    ...
    def vars_for_template(self):
            ...
            if self.player.id_in_group ==1:
                self.player.grole = "Osi Ono"
            else:
                self.player.grole = "Ann-Marie Ryan"
...
class USAMart_materials_no_timer(Page):
    form_model = "player"

    template_name = "BissapBops/USAMart_materials.html"
...
class NJLLC_materials_no_timer(Page):
    form_model = "player"
    template_name = "BissapBops/NJLLC_materials.html"
...
class Negotiated_outcome_two(Page):
    ...
    def is_displayed(self):
        if self.player.id_in_group == 2 and self.group.deal:
            return True
        else:
            return False

class Negotiated_outcome_three(Page):

    form_model = "group"
    form_fields = ["sla","warehouse","contract","location","juice_lines_no_split","spice_no_split"]

    def is_displayed(self):
        if self.player.id_in_group == 2 and self.group.deal and not self.group.split:
            return True
        else:
            return False

class Negotiated_outcome_four(Page):

    form_model = "group"
    form_fields = ["sla","warehouse","contract","location","juice_split1","spice_split1","juice_split2","spice_split2"]

    def is_displayed(self):
        if self.player.id_in_group == 2 and self.group.deal and self.group.split:
            return True
        else:
            return False
...
class Outcome_wait(WaitPage):
    ...
    def vars_for_template(self):
        if self.player.id_in_group == 1:
            return {"title_text": "Reporting the outcome", "body_text":"Wait a moment while Osi finishes inputting the results.\n\n"}
        else:
...
page_sequence = [IntroWaitPage, Introduction, USAMart_materials, NJLLC_materials, Planning_doc, Meeting_wait, Meeting_location_reminder, USAMart_materials_no_timer, NJLLC_materials_no_timer, Negotiated_outcome_one, Negotiated_outcome_two, Negotiated_outcome_three,Negotiated_outcome_four, Finished_case, Journaling_page, Outro]
